...
//
// Run encoding is never chosen on its own. Containers keep the shape an operation
// produced until RunOptimize is called, which is the same bargain the reference C
// implementation strikes: scanning for runs costs a pass over the data, so the
// caller decides when it is worth paying. Operations between two run containers
// stay in interval arithmetic and never materialise the values at all.
//
