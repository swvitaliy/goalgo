...
//
// Everything below that is v2: three container encodings, with run containers
// produced only by an explicit RunOptimize. Serialisation does not change how set
// operations perform, so against v1 and v2 the interesting measurements here are
// ToBytes, FromBytes and the size of what comes out.
//
